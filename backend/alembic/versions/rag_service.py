...
class EmbeddingService:
    ...
    async def _google_embed(self, texts: list[str]) -> list[list[float]]:
        """
        FIX #2: lambda closure bug corregido con default argument (t=text)
        FIX #1: usa settings.GOOGLE_EMBEDDING_MODEL (text-embedding-004)
        """
        import google.generativeai as genai
        genai.configure(api_key=settings.GOOGLE_API_KEY)
        loop = asyncio.get_event_loop()
        results = []
        model_name = settings.GOOGLE_EMBEDDING_MODEL  # "models/text-embedding-001"

        for i in range(0, len(texts), 20):  # lotes de 20 (límite Google)
            batch = texts[i:i+20]
            for text_item in batch:
                # FIX #2: usar t=text_item para capturar valor actual, no referencia
